...
test('ensureSignupPhoneEntryReady falls back to visible login action when signup CTA is collapsed but still present in DOM', async () => {
  const api = new Function(`
const logs = [];
const clicks = [];
let phase = 'entry';
...
const document = {
  ...
  querySelectorAll(selector) {
    ...
    if (selector === 'a, button, [role="button"], [role="link"]') {
      return phase === 'entry' ? [signupButton, loginButton] : [];
    }
    if (selector === 'input') {
      if (phase === 'email') return [emailInput];
      if (phase === 'phone') return [phoneInput];
