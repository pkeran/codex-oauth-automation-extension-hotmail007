...
test('auto-run controller treats structured PHONE_SMS_NO_SUPPLY as round-fatal without relying on message text', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  const controller = api.createAutoRunController({
    ...
    runAutoSequenceFromStep: async () => {
      events.runCalls += 1;
      if (events.runCalls === 1) {
        const error = new Error('structured-no-supply');
        error.code = 'PHONE_SMS_NO_SUPPLY';
        throw error;
      }
    },
    runtime,
  ...
  assert.equal(events.runCalls, 2, 'structured no-supply should fail current round and continue next round');
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'retrying'), false);
  assert.equal(events.accountRecords.length, 1);
  assert.equal(events.accountRecords[0].status, 'failed');
  assert.equal(events.accountRecords[0].reason, 'structured-no-supply');
});
...
test('auto-run controller caps same-round retries for custom mail provider pool email', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  const controller = api.createAutoRunController({
    ...
    isStopError: () => false,
    /*
    isStopError: (error) => (error?.message || String(error || '')) === '娴佺▼宸茶鐢ㄦ埛鍋滄銆?,
    */
    launchAutoRunTimerPlan: async () => false,
    normalizeAutoRunFallbackThreadIntervalMinutes: (value) => Math.max(0, Math.floor(Number(value) || 0)),
    persistAutoRunTimerPlan: async () => ({}),
    ...
    runAutoSequenceFromStep: async () => {
      events.runCalls += 1;
      if (events.runCalls <= 2) {
        throw new Error('Step 3 transient failure while reusing custom pool email.');
      }
    },
    runtime,
    setState: async (updates = {}) => {
    ...
    throwIfAutoRunSessionStopped: (sessionId) => {
      if (sessionId && sessionId !== runtime.state.autoRunSessionId) {
        throw new Error('auto-run stopped');
        /*
        throw new Error('娴佺▼宸茶鐢ㄦ埛鍋滄銆?);
        */
      }
    },
    waitForRunningStepsToFinish: async () => currentState,
  ...
  assert.equal(events.runCalls, 2, 'custom mail provider pool should stop after the configured same-round retry cap');
  assert.equal(events.broadcasts.filter(({ phase }) => phase === 'retrying').length, 1);
  assert.equal(events.accountRecords.length, 1);
  assert.equal(events.accountRecords[0].status, 'failed');
  assert.match(events.accountRecords[0].reason, /Step 3 transient failure/);
  assert.ok(events.logs.some(({ message }) => /重试上限|閲嶈瘯涓婇檺/.test(message)));
  assert.equal(runtime.state.autoRunActive, false);
  assert.equal(runtime.state.autoRunSessionId, 0);
});
