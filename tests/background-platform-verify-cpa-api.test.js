...
test('platform verify module forwards SUB2API account priority to panel submit flow', async () => {
  const source = fs.readFileSync('background/steps/platform-verify.js', 'utf8');
  const sentMessages = [];
  ...
  assert.equal(sentMessages.length, 1);
  assert.equal(sentMessages[0].message.payload.sub2apiAccountPriority, 9);
});
